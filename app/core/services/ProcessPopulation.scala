...
trait ProcessPopulation {
  ...
  private def populateStanza(id: String, stanza: Stanza, process: Process): Either[GuidanceError, Stanza] = {
    ...
    stanza match {
      ...
      case c: ChoiceStanza =>
        Right(Choice(c.copy(tests = c.tests.map(t => t.copy(left = timescales.translate(t.left), right = timescales.translate(t.right))))))
      case c: CalculationStanza =>
        Right(Calculation(c.copy(calcs = c.calcs.map(op => op.copy(left = timescales.translate(op.left), right = timescales.translate(op.right))))))
      case vs: ValueStanza =>
        Right(vs.copy(values = vs.values.map(v => v.copy(value = timescales.translate(v.value)))))
  ...
  private def phrase(phraseIndex: Int, stanzaId: String, process: Process): Either[GuidanceError, Phrase] =
    process.phraseOption(phraseIndex).fold[Either[GuidanceError, Phrase]](Left(PhraseNotFound(stanzaId, phraseIndex))){
      ...
      case p: Phrase => Right(Phrase(timescales.translate(p.english), timescales.translate(p.welsh)))
    }
