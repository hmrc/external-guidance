...
@Singleton
class Rates @Inject() (tp: TodayProvider) extends HmrcLabelledDataExpansion {

  // Page Analysis
  private val SectionId: Int = 1
  private val RateId: Int = 2
  private val YearId: Int = 3
  private val RateRegex = "\\[rate:([^:]+):([^:]+)(?::(\\d\\d\\d\\d))?\\]"

  def referencedRateIds(p: Phrase): List[String] = ??? // (referencedIds(p.english) ++ referencedIds(p.welsh)).distinct
  def referencedRateIds(p: Seq[Phrase]): List[String] = ??? // p.toList.flatMap(referencedIds)

  def referencedNonPhraseRateIds(flow: Map[String, Stanza]): List[String] = ???
