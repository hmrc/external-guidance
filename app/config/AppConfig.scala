...
trait AppConfig {
  ...
  val addWelshToUnauthenticatedGuidance: Boolean
...
class AppConfigImpl @Inject() (config: Configuration, servicesConfig: ServicesConfig) extends AppConfig {
  ...
  lazy val addWelshToUnauthenticatedGuidance: Boolean = config.getOptional[Boolean]("welsh-text.add-to-unauthenticated-guidance").fold(false)(answer => answer)
