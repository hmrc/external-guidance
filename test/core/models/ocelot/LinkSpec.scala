...
class LinkSpec extends BaseSpec {
  ...
  val always = false
  val popUp = false
  val linkStr1: String = s"""{"id":${id1},"dest":"${dest1}","title":"${title1}","window":${window}}"""
  val linkStr2: String = s"""{"id":${id2},"dest":"${dest2}","title":"${title2}","window":${window}}"""
  ...
  "Link" must {
    ...
    "serialise to JSON" in {
      Json.toJson(link1).toString() shouldBe linkStr1
    }
