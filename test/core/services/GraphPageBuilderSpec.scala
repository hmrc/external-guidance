 ...
 val links: Vector[Link] = Vector(Link(0, "http://my.com/news", "MyCOM Daily News", window = true))
  ...
  trait GraphTest {
    ...
    private val flow = Map(
      ...
      "2" -> QuestionStanza(1, Seq(2, 1), Seq("3", "3"), None, stack = false),
      "3" -> ChoiceStanza(Seq(pageId2, pageId4), Seq(choiceTest), stack = false),
      ...
      "16" -> InstructionStanza(3, Seq("161"), None, stack = false),
      "161"-> ValueStanza(List(Value(ScalarType, "error", "0")), Seq("10"), stack = false),
      "10" -> InstructionStanza(2, Seq("100"), None, stack = false),
      "100" -> ChoiceStanza(Seq("161", pageId6, pageId5),Seq(choiceTest), stack = false),
    ...
    val loopFlow = Map(
      pageId1 -> PageStanza("/start", Seq("1000"), stack = false),
      "1000" -> CalloutStanza(Note, 3, Seq("1"), stack = false),
      "1" -> ValueStanza(values, Seq("2"), stack = true),
      "2" -> CalculationStanza(Seq(CalcOperation("[label:ThisYear]", Subtraction, "2013", "YearCount")), Seq("4"), stack = false),
      "4" -> ChoiceStanza(Seq("5", "6"), Seq(ChoiceStanzaTest("[label:index]", LessThanOrEquals, "[label:YearCount]")), stack = false),
      "5" -> CalculationStanza(calcOperations, Seq("4"), stack = false),
      "6" -> InstructionStanza(2, Seq("6000"), None, stack = true),
      "6000" -> PageStanza("/chooser2", Seq("7"), stack = false),
      "7" -> CalloutStanza(Title, 3, Seq("end"), stack = false),
      "end" -> EndStanza
    )
